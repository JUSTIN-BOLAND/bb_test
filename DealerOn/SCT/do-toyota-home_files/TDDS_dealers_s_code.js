...
var tmsomni;
if(!tms_account){
	tms_account = "tdds-prod,tmsglobalprod";
	//var tms_account = "tdds-dev"; //development reporting suite
}
// add global if not already specified
if(tms_account == "tdds-prod"){
	tms_account = "tdds-prod,tmsglobalprod";
}
// add to send GST traffic to their own suite
if ("gst" === tms_region){
	tms_account+=",tdds-gst-prod";
}

tmsomni=s_gi(tms_account);

if(!tms_linkInternalFilters){
	tmsomni.linkInternalFilters = "localhost,javascript:,dealer.com,clickmotivefusiondemo.com";
}else{
...
tmsomni.linkTrackVars="";
tmsomni.linkTrackEvents="";
//tmsomni._linkTrackVars = tmsomni.linkTrackVars;
...
function s_doPlugins(s) {
	// abort if not http
	if(typeof s.abort === "function" && !window.location.protocol.match(/^http/i)){
		s.abort();
	}
	...
	s.prop60 = document.domain;
	s.eVar60 = "D=c60";


	/* Set server */
	/* Dealer Information */
	s.server = tms_dealerCode;
	if (s.server) {
		s.eVar10 = s.server;
		s.prop16 = "D=v10";
	}
	...
	s.eVar12 = tms_tda;
	s.prop12 = "D=v12";
	s.eVar42 = tms_district;
	s.prop42 = "D=v42";
	s.eVar68 = tms_autogroup;
	s.prop68 = "D=v68";
	s.eVar46 = tms_provider;
	s.prop46 = "D=v46";
	...
	if (s.pageName) {
		...
		s.channel = s.channelExtractCust(":", 1, (s.pageName.split(':').length - 1), s.pageName, 1);
		s.eVar24 = "D=ch";
		s.eVar4 = s.channelExtractCust(":", 1, 2, s.pageName, 1);
		s.prop4 = "D=v4";
		s.eVar19 = s.channelExtractCust(":", 1, 3, s.pageName, 1);
		s.prop19 = "D=v19";
		s.eVar11 = s.channelExtractCust(":", 1, 4, s.pageName, 1);
		s.prop11 = "D=v11";
		s.eVar28 = s.channelExtractCust(":", 1, 5, s.pageName, 1);
		s.prop28 = "D=v28";
	}
	if (s.campaign) {
		s.campaign = s.getValOnce(s.campaign, "s_campaign_gvo", 0);
	} else {
		s.campaign = s.getQueryParam('siteid,cid,srchid,cmp,cmpid');
		s.campaign = s.getValOnce(s.campaign, "s_campaign_gvo", 0);
	}
	...
	/* Internal Campaigns */
	if (s.eVar70) {
		s.eVar70 = s.getValOnce(s.eVar70, "s_evar70_gvo", 0);
	} else {
		s.eVar70 = s.getQueryParam('intcmp');
		s.eVar70 = s.getValOnce(s.eVar70, "s_evar70_gvo", 0);
	}
	/* Set Channel Variables */
	s.channelManager('siteid,cid,srchid,cmp');
	if (s._channel) {
		s.prop30 = s._channel;
		s.eVar30 = s.prop30;
		if (s._partner) {
		...
		s.hier4 = s.crossVisitParticipation(s.prop30, 's_prop30_stack', '30', '15', '>', '', 1);
	...
	s.eVar57 = "D=c57";
	s.eVar69 = s.prop69 = s.getVisitNum();
	s.eVar75 = s.prop75 = s.getDaysSinceLastVisit();
	...
	try {
		...
		s.setupLinkTrack("prop15,prop23,prop22,prop49", "SC_LINKS");

		/* Automatically Capture Downloads */
		s.downloadURL = s.downloadLinkHandler();
		if (s.downloadURL) {
			//s.linkTrackVars = s.apl(s.linkTrackVars, "events", ",", 2);
			//s.linkTrackEvents = s.apl(s.events, "event16", ",", 2);
			s.linkName = s.prop17;
			s.prop23 = 'download: ' + s.prop17 + ' | ' + s.downloadURL;
			s.prop65 = s.prop17;
			s.eVar65 = 'D=c65';
		...
		if (s.exitURL) {
			s.linkName = s.prop23;
			s.prop17 = 'exit: ' + s.prop23 + ' | ' + s.exitURL;
	...
	if (s.prop64 && !s.eVar64) {
		s.eVar64 = "D=c64";
	}
	// persist
	s.prop74 = s.eVar74 = s.getAndPersistValue(s.prop34, 's_p34_persist', 0);
	/* Inventory Metrics */
	...
	if (!s.eVar74 && s.prop74) s.eVar74 = 'D=c74';
	if (!s.eVar20 && s.prop20) s.eVar20 = 'D=c20';
  ...
  // overwrite any DWP setting and send everything on links
  s.linkTrackVars = "";
  s.linkTrackEvents = "";
}
